...
# -*- coding: utf-8 -*-
"""
Created on Fri May 15 09:20:56 2020


DCA Loss Function from: https://github.com/theislab/dca/tree/master/dca

version 1.0: Baseline from DCA or Deep Count Autoencoder for RNA Seq. (https://www.nature.com/articles/s41467-018-07931-2)
"""
...
class NB(object):
    ...
    def loss(self, y_true, y_pred, mean=True):
        ...
        with tf.name_scope(self.scope):
            ...
            tf.where(tf.is_nan(self.theta), tf.zeros_like(self.theta), self.theta)
            # Clip theta
        ...
        #with tf.Session() as sess: print(final.eval()) 
        return final
...
class ZINB(NB):
    ...
    def loss(self, y_true, y_pred, mean=True):
        ...
        with tf.name_scope(self.scope):
            ...
            y_true = tf.cast(y_true, tf.float32)
            y_pred = tf.cast(y_pred, tf.float32) * scale_factor
            theta = tf.minimum(self.theta, 1e2)
